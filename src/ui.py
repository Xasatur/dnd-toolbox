...
if tool == "Loot Generator":
    ...
    rarity = st.selectbox("Seltenheit", [1, 2, 3], format_func=lambda x: {1: "gewöhnlich", 2: "selten", 3: "episch"}[x])
    amount = st.slider("Wie viele Items?", 1, 5, 1)

    if st.button("Loot generieren"):
        response = requests.get("http://localhost:8000/loot", params={"rarity": rarity, "amount": amount})
        data = response.json()
